...
export const ClientImpl_WYSIWYG: FC<{
  ...
  useEffect(() => {
    if (elemRef.current !== null) {
      setQuill(q => {
        if (!q) {
          q = elemRef.current !== null ? new Quill(elemRef.current, {
            theme: 'snow',
            modules: {
              toolbar: [
                [{ header: [1, 2, false] }],
  ...
  const onTextChange = useCallback<TextChangeHandler>((ev) => {
  ...
  return (
    <div
      className={ props.editable ? "" : "[&_.ql-editor]:border-t-[1px] [&_.ql-editor]:border-[#ccc] [&_.ql-toolbar]:hidden"}
    >
      <div ref={elemRef}>
